...
AspectIDE is early alpha. Security reports are still welcome, especially issues involving workspace file access, command execution, update/install behavior, extension loading, or AI tools that can mutate files.
...
AspectIDE must never delete user project directories during uninstall, cleanup, update, or cache maintenance.
