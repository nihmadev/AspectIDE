...
const primaryEndpoints = [
  "https://updates.aspect-ide.dev/stable/latest.json",
  "https://updates.aspect-ide.dev/beta/latest.json",
].join(",");
const mismatchedEndpoints = [
  "https://updates.aspect-ide.dev/stable/latest.json",
  "https://updates.aspect-ide.dev/canary/latest.json",
].join(",");
// The free path ships a single GitHub "latest" manifest with no channel segment.
const singleEndpoint = "https://github.com/GofMan5/aspect-ide/releases/latest/download/latest.json";
...
// Free path: updater secrets only, no OS code-signing certs в†’ unsigned installers,
...
function withPolicyCheckout(callback) {
  const checkout = mkdtempSync(join(tmpdir(), "aspect-release-policy-test-"));
  try {
...
function updaterOnlyEnv(updaterEndpoints) {
  return {
    TAURI_SIGNING_PRIVATE_KEY: "AspectUpdaterSigningPrivateKeyMaterial-2026-Prod-9A8B7C6D5E",
    TAURI_SIGNING_PRIVATE_KEY_PASSWORD: "TauriUpdaterKeyPassword-2026-Prod!",
    TAURI_UPDATER_ENDPOINTS: updaterEndpoints,
    TAURI_UPDATER_PUBLIC_KEY: "AspectUpdaterPublicKeyMaterial-2026-Prod-9A8B7C6D5E",
  };
}
...
function releaseEnv(updaterEndpoints) {
  return {
    ...
    APPLE_ID: "release@aspect-ide.dev",
    APPLE_KEYCHAIN_PASSWORD: "AppleKeychainPassword-2026-Prod!",
    APPLE_PASSWORD: "AppleAppPassword-2026-Prod!",
    APPLE_PROVIDER_SHORT_NAME: "ASPECT",
    APPLE_SIGNING_IDENTITY: "Developer ID Application: AspectIDE Team (9A8B7C6D5E)",
    TAURI_SIGNING_PRIVATE_KEY: "AspectUpdaterSigningPrivateKeyMaterial-2026-Prod-9A8B7C6D5E",
    TAURI_SIGNING_PRIVATE_KEY_PASSWORD: "TauriUpdaterKeyPassword-2026-Prod!",
    TAURI_UPDATER_ENDPOINTS: updaterEndpoints,
    TAURI_UPDATER_PUBLIC_KEY: "AspectUpdaterPublicKeyMaterial-2026-Prod-9A8B7C6D5E",
