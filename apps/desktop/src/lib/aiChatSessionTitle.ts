...
export async function generateChatSessionTitle(input: {
  ...
  // Reasoning effort is intentionally NOT forwarded here: titles are generated by
  // the fastest available model (haiku/mini/flash), not the active one, and a
  // 6-word throwaway title gains nothing from reasoning while costing latency/tokens.
  if (isTauriRuntime()) {
    try {
