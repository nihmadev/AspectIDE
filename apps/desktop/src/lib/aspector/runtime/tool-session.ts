...
import { applyGoalToolProgress } from "./../session/goal/session-goal-run";
import { getAiSessionGoal, setAiSessionGoal } from "./../session/goal/session-goal";
import { sanitizeSessionGoal, sanitizeSessionTodos } from "./../session/orchestration/sanitize";
import { replaceAiSessionTodos } from "./../session/todos";
import { spawnSubagent, resolveSubagentType } from "./../subagents/subagents";
import { countRunningSubagents, getSubagentRun } from "./../subagents/runs";
import { resolveMaxParallelSubagents } from "./../subagents/policy";
import { booleanArg, clamp, isRecord, numberArg, stringArg, stringArrayArg, toolJson, topCounts, truncateText, type ToolResult, type UnknownRecord } from "./shared";
import { luxCommands, type McpServerConfig } from "./../../tauri/commands";
...
export async function askUserTool(args: UnknownRecord, input: AiChatSendInput, callId: string): Promise<ToolResult> {
  ...
  const { registerPendingQuestion, waitForQuestionAnswer } = await import("./../utils/pending-question");
...
export async function presentPlanTool(args: UnknownRecord, input: AiChatSendInput, callId: string): Promise<ToolResult> {
  ...
  const { registerPendingPlan } = await import("./../utils/pending-plan");
  registerPendingPlan({
    planId: `plan-${callId}`,
    turnId: callId,
    sessionId: input.chatSessionId,
...
export async function mcpManageTool(args: UnknownRecord, _input: AiChatSendInput): Promise<ToolResult> {
  ...
  const { isTauriRuntime, luxCommands, MCP_SERVERS_KEY } = await import("./../../tauri/commands");
