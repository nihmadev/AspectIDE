...
} from "./../../agent-browser/core";
import { formatBrowserCommandPreview, isReadOnlyBrowserArgs } from "./../../agent-browser/command-catalog";
...
import { booleanArg, clamp, numberArg, stringArg, stringArrayArg, toolJson, type ToolResult, type UnknownRecord } from "./shared";
import { isReadOnlyAgentMode } from "./tools";
import { encodeVisionImageFromDataUrl } from "./../utils/file-context";
import { resolveVisionImageFormatFromPreferences } from "./../utils/vision-format";
...
async function streamPortAfterAction(input: AiChatSendInput) {
  ...
  try {
    ...
    const { bumpBrowserStreamRefresh } = await import("./../chat/turn-runtime");
