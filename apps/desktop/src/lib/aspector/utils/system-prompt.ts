...
import { resolveModelProtocol, type AiAgentMode, type AiModelConfig, type AiPreferences, type AiProviderConfig } from "./preferences";
...
- Use Lux's force multipliers as defaults, not fallbacks — they are why this IDE beats a plain chat agent; the best result reaches for every relevant one, not priors:
  ...
  - WebResearch whenever anything depends on external, current, or uncertain knowledge (library APIs, releases, error messages, versioned behavior) instead of stale memory — then cite [1], [2]; MultiWebResearch for multi-facet questions (2-6 facet queries searched in parallel, merged into one ranked list). ListSkills then UseSkill before improvising a procedure a vetted skill covers.
