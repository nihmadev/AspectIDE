...
function TablePreview({ headers, rowCount, rows, truncated }: { headers: string[]; rows: string[][]; rowCount: number; truncated: boolean }) {
  ...
  const summary = `${t("filePreview.table.rowCount", { count: rowCount })}${truncated ? ` — ${t("filePreview.table.truncatedSuffix")}` : ""}`;
  return (
...
function ArchivePreview({ preview }: { preview: Extract<FilePreview, { kind: "archive" }> }) {
  const { t } = useTranslation();
  const summary = `${t("filePreview.archive.entryCount", { count: preview.total_entries })}${preview.truncated ? ` — ${t("filePreview.table.truncatedSuffix")}` : ""}`;
  return (
