...
import { mapComposerAttachments } from "./AspectorComposerAttachments";
import { AspectorChatComposer } from "./AspectorChatComposer";
import { AspectorChatHistoryPopover } from "./AspectorChatHistoryPopover";
import { AspectorChatMessages, MarkdownSmoothStreamContext } from "./AspectorChatMessages";

import { AspectorAgentOrchestrationRail } from "./AspectorAgentOrchestrationRail";
import { AspectorQuestionCard } from "./AspectorQuestionCard";
import { AspectorPlanCard } from "./AspectorPlanCard";
import { AspectorPlanRunCard, type ActivePlanRun } from "./AspectorPlanRunCard";
...
import { AspectorChatError } from "./AspectorChatErrorNotice";
import { AspectorRetryBanner } from "./AspectorRetryBanner";
...
export function AspectorChatPanel({ embedded = false, presentation = "panel", showCloseButton = true }: AiChatPanelProps) {
  ...
  useAspectUsagePoller();
  const luxideUsageMap = useAspectUsageStore((state) => state.map);
  ...
  const modelOptions = useMemo(() => {
    ...
    for (const provider of aiPreferences.providers) {
      const luxide = isAspectProvider(provider);
      for (const model of provider.models) {
        ...
        options.push({
          ...
          badge: luxide ? aspectWeeklyBadge(usage) ?? undefined : undefined,
          status: luxide ? aspectAvailability(usage) ?? undefined : undefined,
        });
      }
    }
    return options;
  ...
  const handleSend = useCallback(async (
    ...
    try {
      ...
      if (errorPresentation.kind === "auth") {
        ...
        if (prov && isAspectProvider(prov)) void relinkAspect(prov.baseUrl);
      }
    } finally {
      pendingSendLockRef.current.delete(sessionId);
      setSendPhase((current) => (current?.sessionId === sessionId ? null : current));
    ...
    <div className="ai-chat-header-actions">
      {showHeaderSessionChrome && (
        <>
          ...
          <AspectorChatHistoryPopover workspaceRoot={workspace?.root ?? null} />
  ...
  const renderComposerContent = () => (
    <>
      <AspectorChatQueuedMessages sessionId={activeAiChatSessionId} onSendNow={handleQueuedSendNow} t={t} />
      <AspectorChatComposer
  ...
  return (
    <aside className="ai-chat-panel" style={chatFontStyle} aria-label={t("aiChat.panel.aria")} data-empty-home={isAgentHome} data-embedded={embedded} data-presentation={presentation} data-status={activeStatus}>
      ...
      {pendingCrossSessionApproval && (
        <AspectorChatGlobalApprovalBanner
      ...
      >
        ...
        {showOrchestrationRail && activeChatSession && (
          <AspectorAgentOrchestrationRail
            sessionId={activeChatSession.id}
        ...
        <div className="ai-chat-scroll" ref={scrollRef} onScroll={handleBodyScroll}>
          {!showOrchestrationRail && activeChatSession && (
            <AspectorSubagentPanel sessionId={activeChatSession.id} t={t} />
          )}
          {!showOrchestrationRail && (
            <AspectorAutomaticChecklist
              sessionId={activeChatSession?.id ?? ""}
              agentMode={aiPreferences.agentMode}
              t={t}
            />
          ...
          {messages.length > 0 ? (
            <section className="ai-chat-thread" aria-live="polite">
              ...
              {activeChatSession && <AspectorRetryBanner sessionId={activeChatSession.id} history={activeChatSession.errorHistory} t={t} />}
              {activeSessionBusy && activeChatSession && (
                <AspectorAgentNowPlaque sessionId={activeChatSession.id} status={activeStatus} workTail={liveWorkTail} t={t} />
              )}
              {activeSessionClosed && <AspectorChatClosedNotice onRestore={() => restoreAiChatSession(activeAiChatSessionId)} t={t} />}
              {sendError && (
                <AspectorChatError
      ...
      {!isAgentHome && (
        <footer className="ai-chat-composer-shell">
          <AspectorSessionReviewBar sessionId={activeAiChatSessionId} t={t} />
