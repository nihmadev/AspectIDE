...
import {
  ...
  useQueuedMessages,
  type QueuedMessage,
} from "../../lib/aiChatQueue";
import type { TranslateFn } from "../../lib/i18n/useTranslation";

type AiChatQueuedMessagesProps = {
 ...
 */
export function AiChatQueuedMessages({ sessionId, onSendNow, t }: AiChatQueuedMessagesProps) {
    ...
    <div className="ai-chat-queue" aria-label={t("aiChat.queue.aria")}>
      ...
      <div className="ai-chat-queue-list">
        {queued.map((entry, index) => (
          <AiQueuedChip key={entry.id} entry={entry} index={index} onSendNow={onSendNow} t={t} />
        ))}
      </div>
    </div>
  );
...
function AiQueuedChip({ entry, index, onSendNow, t }: { entry: QueuedMessage; index: number; onSendNow: (entry: QueuedMessage) => void; t: TranslateFn }) {
