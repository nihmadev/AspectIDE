...
const ISLAND_FLOAT_POS_KEY = "lux.agentIsland.floatPos";
...
function loadIslandFloatPos(): IslandFloatPos | null {
  ...
  try {
    ...
    // Corrupted entry — fall back to the docked position.
  }
  return null;
}
...
function saveIslandFloatPos(pos: IslandFloatPos | null) {
  ...
  try {
    ...
    // Storage unavailable — the position simply won't survive a restart.
  }
}

type IslandDrag = {
  /** Callback ref — MUST be attached to whichever <aside> currently renders,
 ...
 * dock into a free position, clamped to the chat area (.ai-chat-main — the
 ...
 * unmounts and swaps identity (hidden → overview → subagent window) while this
 * hook lives on — a plain ref + mount-time effect would attach the clamp
...
function useIslandDrag(): IslandDrag {
  ...
  const clampToParent = useCallback((x: number, y: number): IslandFloatPos | null => {
    ...
    // The narrow-panel container query hides the rail (display:none) — its
  ...
  // spot (no jump), or redock if already floating — the keyboard twin of dragging
  ...
  // rail (re)mount — before paint, so a restored stale position never flashes
  // off-screen — then watch BOTH boxes. The parent resizes on window/panel
...
function AiAgentOrchestrationRailBody({
  ...
  // actually pinned a goal, opened tasks or spawned subagents (history counts —
...
function SubagentWindow({ run, t, onBack, drag }: { run: SubagentRun; t: TranslateFn; onBack: () => void; drag: IslandDrag }) {
  ...
  // modal (portal — escapes the island's clamped geometry). Esc collapses it.
