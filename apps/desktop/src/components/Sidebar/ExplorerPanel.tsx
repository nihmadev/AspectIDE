...
// Below this visible-row count the tree renders in normal flow Р Р†Р вЂљРІР‚Сњ the virtual
...
export function ExplorerPanel() {
  ...
  // Git status Р Р†РІР‚В РІР‚в„ў tree decorations (green added, gold modified, red deleted, Р Р†Р вЂљР’В¦),
  ...
  const removeFolderFromWorkspace = useCallback((root: string) => {
    // Only close documents that belong to the folder being removed Р Р†Р вЂљРІР‚Сњ leave
  ...
  // active create-row parent change Р Р†Р вЂљРІР‚Сњ not on selection/drag/git ticks.
...
const FileRow = memo(function FileRow({
  ...
  const guideSegments = useMemo(() => {
    const segs: { level: number }[] = [];
    for (let P = 0; P < depth; P++) {
      if (guideMask & (1 << P)) segs.push({ level: P });
    }
    return segs;
  }, [depth, guideMask]);

  // Handlers are built here from already-stable parent callbacks + this row's own
  // `entry`, so they are recreated only when this row actually re-renders Р Р†Р вЂљРІР‚Сњ which,
  ...
  const onDragStart = (event: DragEvent<HTMLButtonElement>) => {
    ...
    event.dataTransfer.setData("application/x-aspect-path", entry.path);
    event.dataTransfer.setData("application/x-aspect-kind", entry.kind);
  ...
  return (
    <div className="file-row-shell" style={{ "--tree-depth": depth } as CSSProperties}>
      {guideSegments.map((seg) => (
        <span
          key={seg.level}
          className="tree-guide-seg"
          style={{ left: `${16 + seg.level * 14}px` } as CSSProperties}
        />
      ))}
      <button
        className="file-row"
      ...
      >
        ...
        {isDirectory
          ? (() => { const s = getFolderIconSvg(entry.name, expanded); return s ? <img src={s} width={15} height={15} className="folder-icon" alt="" /> : (expanded ? <FolderOpen size={15} className="folder-icon" /> : <Folder size={15} className="folder-icon" />); })()
          : (iconMeta.imgSrc ? <img src={iconMeta.imgSrc} width={15} height={15} className={iconMeta.className} alt="" /> : <iconMeta.Icon size={15} className={iconMeta.className} />)
        }
  ...
  return (
    ...
    >
      ...
      {entry.kind === "directory"
        ? (() => { const s = getFolderIconSvg(entry.name); return s ? <img src={s} width={15} height={15} className="folder-icon" alt="" /> : <Folder size={15} className="folder-icon" />; })()
        : (iconMeta.imgSrc ? <img src={iconMeta.imgSrc} width={15} height={15} className={iconMeta.className} alt="" /> : <iconMeta.Icon size={15} className={iconMeta.className} />)
      }
      <input
        autoFocus
        ...
        <Dialog.Content className="delete-entry-dialog" aria-describedby="delete-entry-description">
          ...
          {entry && (
            <div className="delete-entry-target" title={displayPath(entry.path)}>
              {isDirectory
                ? (() => { const s = getFolderIconSvg(entry.name); return s ? <img src={s} width={16} height={16} className="folder-icon" alt="" /> : <Folder size={16} className="folder-icon" />; })()
                : <Trash2 size={16} />
              }
