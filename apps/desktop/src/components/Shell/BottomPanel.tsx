...
export function BottomPanel({ isMaximized = false, onToggleMaximized }: BottomPanelProps) {
  ...
  // are no sessions yet (keeps the "open terminal Р Р†РІР‚В РІР‚в„ў shell is ready" UX). Guarded
  ...
  useEffect(() => {
    ...
    // The read-only AI mirror doesn't count as a usable shell Р Р†Р вЂљРІР‚Сњ opening the
    // Terminal tab with only "Aspect AI" present must still spawn a real one.
  ...
  return (
    <div className="bottom-panel-pages">
      ...
      {/* Keep the terminal page mounted whenever sessions exist (hidden when another
          tab is active) so switching tabs never disposes the xterms Р Р†Р вЂљРІР‚Сњ disposing and
