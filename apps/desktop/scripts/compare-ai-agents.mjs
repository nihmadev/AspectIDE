...
import { runLuxScenario, scenarios } from "./benchmark-ai-agent.mjs";

const exec = promisify(execCallback);

const includeExternal = process.env.LUX_AI_COMPARE_EXTERNAL === "1";
const keepWorkspace = process.env.LUX_AI_AGENT_BENCH_KEEP === "1";
const externalTimeoutMs = Number(process.env.LUX_AI_COMPARE_TIMEOUT_MS ?? 300_000);

const adapters = [
  { id: "lux", label: "Lux IDE AI", kind: "lux" },
...
const selectedAdapters = includeExternal ? adapters : adapters.filter((adapter) => adapter.id === "lux");

if (!includeExternal) {
  console.log("External CLI adapters are available but disabled. Set LUX_AI_COMPARE_EXTERNAL=1 to run Codex/Claude/OpenCode.");
}
...
for (const adapter of selectedAdapters) {
  ...
  for (const scenario of scenarios) {
    const workspaceRoot = await mkdtemp(resolve(tmpdir(), `lux-ai-compare-${adapter.id}-${scenario.id}-`));
    try {
      await scenario.setup(workspaceRoot);
      const runResult = adapter.kind === "lux"
        ? await runLuxScenario(scenario, workspaceRoot)
...
const luxCard = scorecards.find((card) => card.adapter.id === "lux");
const externalCards = scorecards.filter((card) => !card.skipped && card.adapter.id !== "lux");
if (luxCard && externalCards.length > 0) {
  const bestExternal = externalCards.reduce((best, card) => card.percent > best.percent ? card : best, externalCards[0]);
  console.log(`Best external: ${bestExternal.adapter.label} ${bestExternal.percent}%. Lux: ${luxCard.percent}%.`);
  if (luxCard.percent < bestExternal.percent) process.exitCode = 1;
} else if (!luxCard || luxCard.percent < 90) {
  process.exitCode = 1;
}

async function adapterAvailability(adapter) {
  if (adapter.kind === "lux") return { available: true, version: "local prompt/runtime" };
