...
if (!browserPreviewGate) {
  ...
  if (!gateExpression.includes('import.meta.env.VITE_ASPECT_BROWSER_PREVIEW === "1"')) {
    errors.push("Browser preview runtime must require VITE_ASPECT_BROWSER_PREVIEW=1 explicitly.");
  }
}

if (!browserPreviewScriptSource.includes('VITE_ASPECT_BROWSER_PREVIEW: "1"')) {
  errors.push("Browser preview launcher must set VITE_ASPECT_BROWSER_PREVIEW=1 explicitly.");
}
