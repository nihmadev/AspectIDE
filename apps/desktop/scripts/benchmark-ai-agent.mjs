...
const endpoint = normalizeEndpoint(process.env.ASPECT_AI_PROXY_URL ?? "http://127.0.0.1:8799/v1");
const model = process.env.ASPECT_AI_MODEL ?? "gpt-5.5";
const reasoningEffort = process.env.ASPECT_AI_REASONING ?? "xhigh";
const timeoutMs = Number(process.env.ASPECT_AI_AGENT_BENCH_TIMEOUT_MS ?? 180_000);
const keepWorkspace = process.env.ASPECT_AI_AGENT_BENCH_KEEP === "1";

const { buildASPECTSystemPrompt } = await loadPromptBuilder();
...
if (isMain()) {
  const summary = await runAspectAgentBenchmark({ print: true });
  if (!summary.passedAll) process.exitCode = 1;
}

export async function runAspectAgentBenchmark({ print = true } = {}) {
  const results = [];
  for (const scenario of scenarios) {
    const workspaceRoot = await mkdtemp(resolve(tmpdir(), `aspect-ai-agent-${scenario.id}-`));
    try {
      await scenario.setup(workspaceRoot);
      const result = await runAspectScenario(scenario, workspaceRoot);
...
export async function runAspectScenario(scenario, workspaceRoot) {
  const toolLog = [];
  const messages = [
    { role: "system", content: buildASPECTSystemPrompt(context(workspaceRoot, scenario)) },
