#![allow(dead_code)]
//! Native session state — Rust-only session store.
//!
...
pub fn ai_session_dispose(session_id: String) {
    ...
    crate::aspector::session::jobs::dispose_session(&session_id);
}

// ── Todo helpers (used by turn tools) ──

pub fn current_todo(session_id: &str) -> Option<String> {
    todos().lock().ok().and_then(|t| {
        t.get(session_id.trim()).map(|items| {
            serde_json::to_string(items).unwrap_or_default()
        })
    })
}

pub fn update_todo(session_id: &str, action: &str, content: &str, items: &[String]) {
    let mut todo_list = todos().lock().ok().map(|t| {
        t.get(session_id.trim()).cloned().unwrap_or_default()
    }).unwrap_or_default();
    match action {
        "add" => {
            if !content.is_empty() {
                todo_list.push(SessionTodo {
        ...
        "complete" | "done" => {
            if let Some(item) = todo_list.iter_mut().find(|t| t.content == content) {
                item.status = "completed".to_string();
            }
        }
        "remove" | "delete" => {
            todo_list.retain(|t| t.content != content);
        }
        _ => {}
    }
    if let Ok(mut t) = todos().lock() {
        t.insert(session_id.trim().to_string(), todo_list);
    }
}

// ── Plan helpers ──

pub fn current_plan(session_id: &str) -> Option<crate::aspector::plan::Plan> {
    let _ = session_id;
    None
}

pub fn current_plan_raw(session_id: &str) -> Option<serde_json::Value> {
    let _ = session_id;
    None
}
...
pub fn terminal_context(_session_id: &str, _max_lines: usize) -> String {
    String::new()
}
