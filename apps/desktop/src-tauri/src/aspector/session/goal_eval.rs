...
pub async fn ai_goal_eval_verdict(input: GoalEvalInput) -> Result<Option<GoalEvalVerdict>, String> {
    ...
    crate::aspector::transport::apply_temperature(&mut payload, input.reasoning.as_ref(), 0.1);
    ...
    match crate::aspector::transport::completion_streaming(request, |_, _| {}, || false, |_| {}, |_| {})
        .await
    {
        Ok(response) => {
