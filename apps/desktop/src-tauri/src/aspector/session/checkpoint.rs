...
fn ws_key(root: &str) -> String {
    crate::aspector::context::semantic::normalize_slashes_pub(root.trim_end_matches('/')).to_lowercase()
}
...
pub async fn ai_checkpoint(
    ...
    match act {
        ...
        "restore" => {
            ...
            let mut operations: Vec<crate::aspector::tools::executors::AiFilePatchOperation> = Vec::new();
...
pub async fn augment_checkpoint(
    ...
    let (max_bytes, max_files_cap, existing, current_count) = {
        ...
        let existing: std::collections::HashSet<String> = cp
            .files
            .iter()
            .map(|f| crate::aspector::context::semantic::normalize_slashes_pub(&f.path).to_lowercase())
            .collect();
        (
            cp.max_bytes_per_file,
            cp.max_files,
    ...
    for snap in snapshots {
        let key = crate::aspector::context::semantic::normalize_slashes_pub(&snap.path).to_lowercase();
        if cp
            .files
            .iter()
            .any(|f| crate::aspector::context::semantic::normalize_slashes_pub(&f.path).to_lowercase() == key)
        {
            dedup_skipped += 1;
            continue;
        }
...
fn resolve_in_workspace(path: &str, root: &str) -> Option<String> {
    ...
    let normalized = crate::aspector::context::semantic::normalize_slashes_pub(path.trim());
    let root_normalized = crate::aspector::context::semantic::normalize_slashes_pub(root);
    ...
    let resolved_normalized = crate::aspector::context::semantic::normalize_slashes_pub(&resolved.to_string_lossy());
        ...
        if resolved.exists() {
            if let (Ok(canon), Ok(root_canon)) = (
                ...
                if canon.starts_with(&root_canon) {
                    return canon.to_str().map(crate::aspector::context::semantic::normalize_slashes_pub);
                }
                return None;
            }
        }
...
fn editor_snapshot(
    state: &State<'_, SharedState>,
    path: &str,
) -> Option<aspect_core::DocumentSnapshot> {
    let documents = state.documents.lock().ok()?;
    documents.snapshots().into_iter().find(|doc| {
        doc.path.as_ref().is_some_and(|p| {
            crate::aspector::context::semantic::normalize_slashes_pub(&p.to_string_lossy()).eq_ignore_ascii_case(path)
        })
    })
}
