...
//! stories, barrels, entrypoints) plus query-token hits. Composes `aspect_fs`
...
impl Desc {
    fn new(path: &str, root: &str) -> Self {
        ...
        let root = crate::aspector::context::semantic::normalize_slashes_pub(root.trim_end_matches('/'));
        ...
        let extension = crate::aspector::context::semantic::file_extension_pub(&basename_lower);
        let stem_lower =
            basename_lower[..basename_lower.len().saturating_sub(extension.len())].to_string();
        let family_stem = crate::aspector::context::semantic::family_stem_pub(&basename);
        Self {
...
pub async fn ai_related_files(
    ...
    let target_path = path
        .map(|p| crate::aspector::context::semantic::normalize_slashes_pub(p.trim()))
        .filter(|p| !p.is_empty())
    ...
    for entry in &files {
        if !matches!(entry.kind, aspect_core::FsEntryKind::File) {
            continue;
        }
        let entry_path = crate::aspector::context::semantic::normalize_slashes_pub(&entry.path.to_string_lossy());
        if crate::aspector::context::semantic::is_low_signal_path_pub(&entry_path) {
            continue;
        }
    ...
    let target_info = target.map(|t| TargetInfo {
        ...
        basename: t.basename,
        family_stem: crate::aspector::context::semantic::family_stem_pub(&t.basename_lower),
    });
