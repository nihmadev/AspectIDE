//! Windows shell integration: an `aspect` launcher on the user PATH and an
//! Explorer right-click "Open with AspectIDE" verb for folders.
//!
...
/// Records a folder passed on the command line (Explorer verb / `aspect .`) so the
...
pub fn apply_default_integration() {
    if cfg!(debug_assertions) {
        // Dev builds would register target/debug/aspect-desktop.exe — never do that.
        return;
    }
...
mod imp {
    ...
    const MENU_KEY_DIR: &str = r"Software\Classes\Directory\shell\AspectIDE";
    const MENU_KEY_BG: &str = r"Software\Classes\Directory\Background\shell\AspectIDE";
    const MENU_LABEL: &str = "Open with AspectIDE";
    ...
    /// `%LOCALAPPDATA%\AspectIDE\bin` — holds the `aspect.cmd` shim the PATH entry
    /// points at, so PATH itself never has to change across app updates.
    fn shim_dir() -> Result<PathBuf, String> {
        let base = std::env::var_os("LOCALAPPDATA")
            .ok_or_else(|| "LOCALAPPDATA is not set".to_string())?;
        Ok(PathBuf::from(base).join("AspectIDE").join("bin"))
    }
    ...
    fn write_shim(dir: &std::path::Path) -> Result<(), String> {
        ...
        // folder/file arguments so `aspect .` opens the current directory.
        let script = format!("@echo off\r\nstart \"\" \"{}\" %*\r\n", exe.display());
        std::fs::write(dir.join("aspect.cmd"), script)
            .map_err(|error| format!("cannot write aspect.cmd: {error}"))
    }
