...
use aspect_core::{
    ...
    LspSignatureHelp, LspTextEdit, AspectEvent, TextEdit, WorkspaceDiagnostic,
};
...
pub async fn lsp_servers(
    ...
    let extra_dirs = aspect_runtimes::lsp::managed_bin_dirs(&app_data_dir);
    let servers = tokio::task::spawn_blocking(move || {
        aspect_lsp::workspace_language_servers_with_dirs(root, &extra_dirs)
    })
    .await
    ...
    let missing_diagnostics = aspect_lsp::language_server_diagnostics(&servers);
    ...
    let runtime_path_dirs = aspect_runtimes::runtime::runtime_bin_dirs(&app_data_dir);
...
async fn start_servers_background(
    ...
    // Guard the handshake against an unwinding panic deep in `aspect_lsp` (e.g. an
    ...
    let mut status_diagnostics = aspect_lsp::language_server_diagnostics(servers);
...
pub async fn lsp_workspace_symbols(
    state: State<'_, SharedState>,
    query: String,
) -> Result<Vec<aspect_core::LspWorkspaceSymbol>, String> {
    let mut lsp = state.lsp.lock().await;
    let Some(manager) = lsp.as_mut() else {
        return Ok(Vec::new());
    };
...
const STATUS_DIAGNOSTIC_SOURCE: &str = "aspect-lsp";
...
const FORWARDING_DIAGNOSTIC_SOURCE: &str = "aspect-lsp-doc";
...
/// server-status entries (`source == "aspect-lsp"`) and leaves any real per-path
...
fn replace_status_diagnostics_owned(
    ...
    for (path, path_diagnostics) in affected {
        emit_event(
            app,
            AspectEvent::EditorDiagnosticsChanged {
                path,
...
pub fn apply_diagnostics_update(
    app: &AppHandle,
    state: &SharedState,
    update: aspect_lsp::DiagnosticsUpdate,
) -> Result<(), String> {
    {
        let mut current = state.diagnostics.lock().map_err(lock_error)?;
...
async fn forward_document_open_owned(
    ...
    if let (Err(error), Some(path)) = (result, document.path.clone()) {
        let _ = apply_diagnostics_update(
            app,
            state,
            aspect_lsp::DiagnosticsUpdate {
                path: path.clone(),
                diagnostics: vec![WorkspaceDiagnostic {
                    path,
                    line: 1,
                    column: 1,
                    severity: aspect_core::DiagnosticSeverity::Warning,
