...
use aspect_core::{
    ...
    FileViewStrategy, LspWorkspaceEdit, AspectEvent, TextEdit, WorkspaceEditResult,
};
...
pub async fn editor_apply_edits(
    ...
    emit_event(
        &app,
        AspectEvent::EditorDocumentEdited {
...
pub async fn apply_workspace_edit(
    ...
    if !edited_documents.is_empty() {
        emit_event(
            app,
            AspectEvent::EditorDocumentsChanged {
...
async fn apply_workspace_edit_files(
    ...
    for (path, text_edits) in planned {
        ...
        tokio::task::spawn_blocking(move || -> aspect_core::AppResult<()> {
            let mut text = std::fs::read_to_string(&write_path)?;
            aspect_editor::apply_text_edits(&mut text, &text_edits_for_file)?;
        ...
        .map_err(|error| error.to_string())?
        .map_err(String::from)?;
        emit_event(app, AspectEvent::FsChanged { path: path.clone() })?;
        changed_paths.push(path);
    }
...
async fn save_document_to_path(
    ...
    state: State<'_, SharedState>,
    buffer_id: BufferId,
    payload: aspect_editor::DocumentSavePayload,
    save_path: PathBuf,
    attach_path: bool,
) -> Result<DocumentSnapshot, String> {
...
) -> aspect_core::AppResult<String> {
    match strategy {
        FileViewStrategy::SpreadsheetEditor => aspect_file_intel::spreadsheet_edit_text(path),
        FileViewStrategy::TableEditor => aspect_file_intel::table_edit_text(path),
        _ => std::fs::read_to_string(path).map_err(aspect_core::AppError::Io),
    }
}
...
fn write_editor_text(
    ...
    strategy: FileViewStrategy,
    text: &str,
) -> aspect_core::AppResult<Option<std::path::PathBuf>> {
    match strategy {
        FileViewStrategy::SpreadsheetEditor => {
            let saved_path = aspect_file_intel::spreadsheet_write_from_text(path, text)?;
            if saved_path == path {
                Ok(None)
            } else {
                Ok(Some(saved_path))
        ...
        FileViewStrategy::TableEditor => {
            aspect_file_intel::table_write_from_text(path, text)?;
            Ok(None)
        }
        _ => {
            std::fs::write(path, text).map_err(aspect_core::AppError::Io)?;
            Ok(None)
        }
    }
}
