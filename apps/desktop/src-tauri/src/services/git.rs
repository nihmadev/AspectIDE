...
use aspect_core::{GitDiff, GitStatus};
use serde::Serialize;
use tauri::State;
...
pub async fn git_status(state: State<'_, SharedState>) -> Result<GitStatus, String> {
    ...
    status_coalescer()
        .get(root.clone(), move || {
            run_blocking(move || aspect_git::status(root))
        })
        .await
}
...
pub async fn git_diff(state: State<'_, SharedState>) -> Result<GitDiff, String> {
    ...
    diff_coalescer()
        .get(root.clone(), move || {
            run_blocking(move || aspect_git::diff(root))
        })
        .await
}
...
// the new state immediately instead of a ≤300ms-stale snapshot. aspect-git recomputes
...
pub async fn git_stage(
    ...
    let status = run_blocking(move || aspect_git::stage(target, &paths)).await?;
...
pub async fn git_unstage(
    ...
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::unstage(target, &paths)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_discard(
    ...
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::discard(target, &paths)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_commit(
    ...
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::commit(target, &trimmed)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_push(state: State<'_, SharedState>) -> Result<GitStatus, String> {
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::push(target)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_pull(state: State<'_, SharedState>) -> Result<GitStatus, String> {
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::pull(target)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_branches(state: State<'_, SharedState>) -> Result<Vec<String>, String> {
    let root = workspace_root(&state)?;
    run_blocking(move || aspect_git::branches(root)).await
}
...
pub async fn git_checkout_branch(
    ...
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::checkout_branch(target, &name)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_create_branch(
    ...
    let root = workspace_root(&state)?;
    let target = root.clone();
    let status = run_blocking(move || aspect_git::create_branch(target, &name)).await?;
    invalidate_git_caches(&root);
    Ok(status)
}
...
pub async fn git_file_diff(
    ...
    let (head_text, working_text) =
        run_blocking(move || aspect_git::file_diff(root, &diff_path)).await?;
    Ok(GitFileDiff {
        path,
        head_text,
        working_text,
...
where
    T: Send + 'static,
    F: FnOnce() -> aspect_core::AppResult<T> + Send + 'static,
{
