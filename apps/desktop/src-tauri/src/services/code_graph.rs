...
//! [`aspect_codegraph`] crate. Progress events stream on `aspect://code-graph`; the
...
const GRAPH_EVENT: &str = "aspect://code-graph";

/// Location of the persistent parse cache for `root`:
/// `<root>/.aspect/cache/code-graph.bin`. `.aspect` is hidden, so [`aspect_codegraph`]'s own
/// walk skips it — the cache never becomes a graph node or retriggers a rebuild.
///
/// Building through [`aspect_codegraph::Index::build_cached`] against this path makes a
...
fn cache_path(root: &Path) -> PathBuf {
    root.join(".aspect").join("cache").join("code-graph.bin")
}
...
fn build_and_cache(root: &Path) -> Result<aspect_codegraph::Index, aspect_codegraph::IndexError> {
    let cache = cache_path(root);
    let mut index = aspect_codegraph::Index::build_cached(root, &cache)?;
...
pub async fn with_index<F, T>(state: &crate::SharedState, f: F) -> Result<T, String>
where
    F: FnOnce(&aspect_codegraph::Index) -> Result<T, String>,
{
...
fn exact_symbol_ids<'g>(
    graph: &'g aspect_codegraph::CodeGraph,
    token: &str,
) -> &'g [aspect_codegraph::NodeId] {
...
#[allow(dead_code)]
fn node_location(
    graph: &aspect_codegraph::CodeGraph,
    id: aspect_codegraph::NodeId,
...
/// locking wrapper so tests can exercise it against a real [`aspect_codegraph::Index`]
/// without a `SharedState`.
fn fast_context_payload(index: &aspect_codegraph::Index, query: &str) -> serde_json::Value {
    ...
    for token in identifier_tokens(query, 8) {
        ...
        symbols.push(serde_json::json!({
            ...
            "file": file,
            "line": line,
            "callers": aspect_codegraph::callers(graph, id).len(),
            "callees": aspect_codegraph::callees(graph, id).len(),
            "matchCount": ids.len(),
        }));
        if symbols.len() >= 4 {
            break;
...
/// through the graph (name/file/line + caller/owner counts). Returns `None`
...
pub async fn fast_context_part(state: &crate::SharedState, query: &str) -> Option<String> {
    let guard = state.code_graph.lock().await;
...
/// real [`aspect_codegraph::Index`] without a `SharedState`.
fn symbol_hint(index: &aspect_codegraph::Index, query: &str) -> Option<String> {
    let q = query.trim();
    if q.chars().count() < 3
        || q.len() > 128
    ...
    match ids {
        ...
        [only] => {
            ...
            let callers = aspect_codegraph::callers(graph, *only).len();
            let callees = aspect_codegraph::callees(graph, *only).len();
            Some(format!(
...
pub async fn grep_symbol_hint(state: &crate::SharedState, query: &str) -> Option<String> {
    let guard = state.code_graph.lock().await;
...
pub async fn code_graph_query(
    ...
    with_index(&state, |index| {
        let graph = index.graph();
        let Some(node_ref) = aspect_codegraph::resolve_one(graph, &symbol) else {
        ...
        let neighbors = aspect_codegraph::neighbors(graph, node, None)
            .into_iter()
        ...
        let explanation = aspect_codegraph::explain(graph, node).map(|e| ExploreEntry {
        ...
        Ok(CodeGraphQueryResult {
            ...
            callers: aspect_codegraph::callers(graph, node)
                .into_iter()
                .map(|r| QueryNode {
                    name: r.name,
                    file: r.file,
                ...
                .collect(),
            callees: aspect_codegraph::callees(graph, node)
                .into_iter()
                .map(|r| QueryNode {
                    name: r.name,
                    file: r.file,
...
/// under `<workspace>/.aspect/`, returning the absolute path written. The `.aspect`
/// directory is hidden, so [`aspect_codegraph`]'s own ignore policy skips it — the
...
pub async fn code_graph_export_html(state: State<'_, SharedState>) -> Result<String, String> {
    ...
    let html = with_index(&state, |index| {
        let graph = index.graph();
        let communities = aspect_codegraph::detect_communities(graph);
        Ok(aspect_codegraph::to_graph_html(
            graph,
            &communities,
            &root,
            &title,
    ...
    let dir = root.join(".aspect");
    let path = dir.join("code-graph.html");
    let write_path = path.clone();
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        std::fs::create_dir_all(&dir)
            .map_err(|e| format!("Could not create .aspect directory: {e}"))?;
