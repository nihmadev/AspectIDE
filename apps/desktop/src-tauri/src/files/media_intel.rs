...
use crate::system::voice::{transcribe_local_blocking, VoiceTranscriptionRequest};
    ...
    let frame_data_urls = if is_video_extension(&ext) {
        match extract_video_frame_data_urls(&request.path, request.max_frames.unwrap_or(3)) {
            Ok(frames) => {
                if frames.is_empty() {
                    notes.push(
                        "Video frame snapshots unavailable. Install ffmpeg or set ASPECT_FFMPEG_COMMAND."
                            .to_string(),
                    );
                }
                frames
...
fn extract_video_frame_data_urls(path: &Path, max_frames: u8) -> Result<Vec<String>, String> {
    ...
    let ffmpeg = env::var("ASPECT_FFMPEG_COMMAND")
        .ok()
    ...
    let output_dir = env::temp_dir().join(format!("aspect-media-frames-{}", Uuid::new_v4()));
