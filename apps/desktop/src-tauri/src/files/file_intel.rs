...
use aspect_core::{FileFormatSupport, FileInspection, FileInspectionOptions};
use serde::Serialize;
use tauri::State;

use crate::{
    files::media_intel::{build_media_ai_context, FileMediaAiContextRequest, FileMediaAiContextResponse},
...
pub fn file_supported_formats() -> Result<Vec<FileFormatSupport>, String> {
    Ok(aspect_file_intel::supported_formats())
}
...
pub async fn file_inspect(
    ...
    tokio::task::spawn_blocking(move || {
        aspect_file_intel::inspect_file(path, &options.unwrap_or_default())
    })
    .await
