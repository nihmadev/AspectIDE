//! Web research engine — the network glue around [`aspect_research`].
//!
...
//! the top result pages concurrently through the SSRF-guarded [`crate::network::web_fetch`]
...
use crate::{network::web_fetch, SharedState};
...
fn configured_searxng_url(state: &State<'_, SharedState>) -> Option<String> {
    ...
    let value = settings
        .as_ref()?
        .get(aspect_core::SettingsScope::User, SEARXNG_URL_KEY)?;
