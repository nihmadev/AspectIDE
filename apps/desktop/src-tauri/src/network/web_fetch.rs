...
const USER_AGENT: &str = "AspectIDE-WebFetch/0.1";
/// Browser-like UA for SEARCH-PROVIDER requests only вЂ” `DuckDuckGo`/`SearxNG` serve an
...
pub async fn fetch(
    ...
    // capped during streaming вЂ” so rebinding (H2), redirect-to-private (H3) and
...
async fn fetch_search_text(
    ...
    // `allow_private` here is NOT model-controlled вЂ” it is set from user config
...
/// (H2 rebinding). Redirects are followed manually вЂ” `Policy::none()` вЂ” so each
...
async fn resolve_and_screen(
    ...
    // A bare IP literal needs no DNS вЂ” screen it directly.
    ...
    match screen_resolved_addresses(&addresses, allow_private) {
        ...
        // Every flagged address sits in the RFC 2544 benchmark range вЂ” the
...
enum ScreenVerdict {
    Allow,
    Reject,
    /// Only benchmark-range (198.18.0.0/15) addresses were flagged вЂ” allowed
...
/// (fake-ip mode) does that вЂ” real DNS never returns the benchmark range for
...
/// 198.18.0.0/15 вЂ” RFC 2544 benchmarking. Blocked by default, but ALSO the
/// standard fake-IP range TUN-mode proxies (Clash, sing-box) hand out from
/// their synthetic DNS вЂ” see the fake-IP carve-out in [`resolve_and_screen`].
...
const fn is_private_ipv4(ip: Ipv4Addr) -> bool {
    ip.is_private()
        ...
        || ip.is_documentation()
        || ip.is_unspecified()
        // 100.64.0.0/10 вЂ” CGNAT (RFC 6598); not "private" per std but never public.
        || (ip.octets()[0] == 100 && (ip.octets()[1] & 0xc0) == 0x40)
        // 192.0.0.0/24 вЂ” IETF protocol assignments (RFC 6890).
...
const fn embedded_ipv4(segments: [u16; 8]) -> Option<Ipv4Addr> {
    ...
    let is_mapped = marker == 0xffff; // ::ffff:a.b.c.d
                                      // ::a.b.c.d вЂ” exclude the `::`/`::1` specials (not real embedded IPv4).
