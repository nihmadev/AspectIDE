//! Lux IDE managed-gateway device linking.
//!
//! The open-source client ships no API key. To use the bundled Lux IDE provider a
...
#[tauri::command]
pub fn luxide_open_url(app: tauri::AppHandle, url: String) -> Result<(), String> {
    let u = url.trim();
...
#[tauri::command]
pub async fn luxide_link_start(base_url: String) -> Result<LinkStart, String> {
...
#[tauri::command]
pub async fn luxide_link_poll(base_url: String, code: String) -> Result<LinkStatus, String> {
    let origin = gateway_origin(&base_url)?;
...
#[derive(Debug, Clone, Deserialize, Serialize)]
pub struct LuxideModelUsage {
    pub id: String,
    #[serde(default)]
    pub total: i64,
    #[serde(default)]
    pub windows: Vec<LuxideWindowUsage>,
}
...
#[tauri::command]
pub async fn luxide_usage(
    base_url: String,
    token: String,
) -> Result<Vec<LuxideModelUsage>, String> {
