...
pub async fn debug_start(
    ...
    let adapter = tokio::task::spawn_blocking(move || {
        lux_dap::workspace_debug_adapter_for_configuration(adapter_root, &adapter_configuration)
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(String::from)?
...
pub fn apply_debug_update(
    app: &AppHandle,
    update: lux_dap::DebugSessionUpdate,
) -> Result<(), String> {
    match update {
        lux_dap::DebugSessionUpdate::Changed(session) => {
            emit_event(app, lux_core::LuxEvent::DebugSessionChanged { session })
        }
        lux_dap::DebugSessionUpdate::BreakpointsChanged(update) => {
            emit_event(app, lux_core::LuxEvent::DebugBreakpointsChanged { update })
        }
        lux_dap::DebugSessionUpdate::Output { session_id, text } => {
