...
mod tests {
    ...
    #[test]
    fn parses_python_traceback_and_ranks_deepest_workspace_frame() {
        let log = r#"Traceback (most recent call last):
  File "C:\Python312\lib\runpy.py", line 196, in _run_module_as_main
    return _run_code(code, main_globals, None,
  File "app/main.py", line 42, in handler
    total = compute(order)
  File "app/pricing.py", line 17, in compute
    return 1 / quantity
ZeroDivisionError: division by zero"#;
        let analysis = analyze(log, None, 10);
        assert_eq!(analysis.error_class.as_deref(), Some("ZeroDivisionError"));
        assert_eq!(analysis.error_message.as_deref(), Some("division by zero"));
        let best = &analysis.candidates[0];
        assert_eq!(best.path, "app/pricing.py");
        assert_eq!(best.line, Some(17));
        assert!(
            best.in_workspace,
            "relative workspace frame must outrank stdlib"
        );
        assert!(analysis.summary.contains("app/pricing.py:17"));
    }

    #[test]
    fn parses_new_format_rust_panic_with_assertion() {
        let log = "thread 'tests::totals' panicked at crates/billing/src/lib.rs:88:9:\nassertion `left == right` failed\n  left: 41\n  right: 42\nnote: run with `RUST_BACKTRACE=1`";
        let analysis = analyze(log, None, 10);
        assert_eq!(analysis.error_class.as_deref(), Some("panic"));
        assert_eq!(analysis.candidates[0].path, "crates/billing/src/lib.rs");
        assert_eq!(analysis.candidates[0].line, Some(88));
        assert_eq!(analysis.candidates[0].column, Some(9));
        let assertion = analysis.assertion.expect("assert_eq payload extracted");
        assert_eq!(assertion.actual.as_deref(), Some("41"));
        assert_eq!(assertion.expected.as_deref(), Some("42"));
    }

    #[test]
    fn parses_node_stack_and_skips_node_internals() {
        let log = "TypeError: cb is not a function\n    at run (src/queue.js:31:5)\n    at node:internal/process/task_queues:96:5";
        let analysis = analyze(log, None, 10);
        assert_eq!(analysis.error_class.as_deref(), Some("TypeError"));
        let best = &analysis.candidates[0];
        assert_eq!(best.path, "src/queue.js");
        assert_eq!(best.line, Some(31));
        assert!(best.in_workspace);
    }

    #[test]
    fn parses_tsc_diagnostics_and_exit_codes() {
        let log = "src/app/store.ts(120,7): error TS2345: Argument of type 'string' is not assignable.\nProcess completed with exit code 2";
        let analysis = analyze(log, None, 10);
        assert_eq!(analysis.candidates[0].path, "src/app/store.ts");
        assert_eq!(analysis.candidates[0].line, Some(120));
        assert_eq!(analysis.exit_codes, vec![2]);
        assert_eq!(analysis.error_class.as_deref(), Some("typescript error"));
    }

    #[test]
    fn unrecognized_log_reports_honestly() {
        let analysis = analyze("everything is fine, just words", None, 10);
        assert!(analysis.candidates.is_empty());
        assert!(analysis.summary.contains("no structured failure shape"));
        assert!(!analysis.next_actions.is_empty());
    }
}
