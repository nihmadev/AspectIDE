...
//! across the project (`<workspace>/.aspect/skills`) and global (`<app_config>/skills`)
//! scopes. Project skills shadow global ones of the same slug.

use std::path::PathBuf;

use aspect_skills::{ScoredSkill, Skill, SkillDraft, SkillScope};
use serde::Serialize;
...
/// skill вЂ” a repository-controlled `<workspace>/.aspect/skills` skill can therefore no
...
fn discovery_roots(
    ...
    if let Ok(root) = workspace_root(state) {
        roots.push((SkillScope::Project, root.join(".aspect").join("skills")));
    }
    Ok(roots)
}
...
fn scope_root(
    ...
    match scope {
        SkillScope::Project => {
            ...
            Ok(root.join(".aspect").join("skills"))
        }
...
pub fn skills_list(app: AppHandle, state: State<'_, SharedState>) -> Result<Vec<Skill>, String> {
    ...
    aspect_skills::discover_skills(&roots).map_err(String::from)
}
...
pub fn skills_get(
    ...
    let roots = discovery_roots(&app, &state)?;
    aspect_skills::read_skill(&roots, &slug).map_err(String::from)
}

/// Load a skill for the agent-facing `UseSkill` tool, honouring the `enabled`
/// flag вЂ” unlike [`skills_get`], which the Settings UI relies on to load *any*
...
#[allow(dead_code)]
/// (`skills_for` / `aspect_skills::match_skills` filter on `enabled`) and from
...
/// disabled so the caller reports it instead of running its body.
...
pub fn skills_match(
    ...
    let roots = discovery_roots(&app, &state)?;
    let skills = aspect_skills::discover_skills(&roots).map_err(String::from)?;
    Ok(aspect_skills::match_skills(
        &skills,
        &query,
        limit.unwrap_or(5),
    ))
...
pub fn skills_save(
    ...
    if !aspect_skills::is_valid_slug(&slug) {
        return Err(format!("invalid skill name: {slug}"));
    }
    let root = scope_root(&app, &state, scope)?;
    let content = aspect_skills::render_skill_markdown(&draft);
    aspect_skills::write_skill(&root, &slug, &content).map_err(String::from)?;
    aspect_skills::read_skill(&[(scope, root)], &slug)
        .map_err(String::from)?
...
pub fn skills_delete(
    ...
    let root = scope_root(&app, &state, scope)?;
    aspect_skills::delete_skill(&root, &slug).map_err(String::from)
}

/// Toggle a skill's `enabled` flag in place вЂ” preserves the rest of the file
...
pub fn skills_set_enabled(
    ...
    let root = scope_root(&app, &state, scope)?;
    aspect_skills::set_skill_enabled(&root, &slug, enabled).map_err(String::from)
}

/// A skill found in an external tool's directory (Claude/Codex), offered for
/// one-click import into AspectIDE's own skill library.
...
pub struct ImportableSkill {
    ...
    /// Suggested target scope (global source в†’ Global, project source в†’ Project).
...
/// first-scanned wins) вЂ” matching that import writes one file per slug anyway.
fn discover_external_root(hint: SkillScope, root: &PathBuf) -> Vec<aspect_skills::Skill> {
    use std::collections::BTreeMap;
    let mut by_slug: BTreeMap<String, aspect_skills::Skill> = BTreeMap::new();
    if let Ok(flat) = aspect_skills::discover_skills(&[(hint, root.clone())]) {
        for skill in flat {
    ...
    if let Ok(entries) = std::fs::read_dir(root) {
        for entry in entries.flatten() {
            ...
            // Skip dirs that are themselves a skill (have a SKILL.md) вЂ” already
            // captured by the flat pass; only descend into category folders.
            if path.join("SKILL.md").is_file() || path.join("skill.md").is_file() {
                continue;
            }
            if let Ok(nested) = aspect_skills::discover_skills(&[(hint, path)]) {
                for skill in nested {
                    by_slug.entry(skill.slug.clone()).or_insert(skill);
                }
            }
...
/// `OpenClaw`, Hermes) so the user can import them вЂ” individually or all at once вЂ”
...
/// Import a skill into AspectIDE's library from raw `SKILL.md` content (a discovered
