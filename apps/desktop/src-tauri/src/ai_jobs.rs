...
//!   returns immediately and the subagent's model↔tool loop runs in a spawned
...
//!   runs detached, its live output still mirrors into the "Lux AI" terminal
...
mod tests {
    ...
    fn task(agent_id: &str, status: JobStatus) -> BackgroundTask {
        BackgroundTask {
            agent_id: agent_id.to_string(),
            call_id: format!("call-{agent_id}"),
    ...
    #[test]
    fn task_lifecycle_and_eviction_keeps_running_entries() {
        let session = "jobs-test-session-1";
        for index in 0..(MAX_TASKS_PER_SESSION + 3) {
            let status = if index == 0 {
                JobStatus::Running
            } else {
                JobStatus::Done
            };
            register_task(session, task(&format!("agent-{index}"), status));
        }
        let listed = list_tasks(session);
        assert_eq!(listed.len(), MAX_TASKS_PER_SESSION);
        // The running entry survived eviction even though it is the oldest.
        assert!(listed.iter().any(|entry| entry.agent_id == "agent-0"));
        ...
        let updated = list_tasks(session);
        let done = updated
            .iter()
            .find(|entry| entry.agent_id == "agent-0")
            .unwrap();
        assert_eq!(done.status, JobStatus::Done);
        assert_eq!(done.summary, "all done");
        assert_eq!(done.board_posts, 2);
        dispose_session(session);
        assert!(list_tasks(session).is_empty());
    }

    #[test]
    fn shell_job_result_is_tail_clamped() {
        let session = "jobs-test-session-2";
        register_shell_job(session, "job-1", "cargo test");
        let long = "x".repeat(MAX_SHELL_RESULT_CHARS + 500);
        complete_shell_job(session, "job-1", JobStatus::Done, long);
        let job = get_shell_job(session, "job-1").unwrap();
        assert_eq!(job.status, JobStatus::Done);
        assert!(job.result.starts_with("[…"));
        assert!(job.result.len() <= MAX_SHELL_RESULT_CHARS + 64);
        dispose_session(session);
        assert!(get_shell_job(session, "job-1").is_none());
    }
}
