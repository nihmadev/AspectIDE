# Local channel policy вЂ” zero listening ports

AspectIDE ships with **no listening ports in the core**. This is a deliberate security
...
|---|---|---|
| UI в†” Rust | Native Tauri IPC (`tauri://` assets in production) | none |
...
| DAP adapter (CodeLLDB) | TCP-only adapter: binds `127.0.0.1:0` (ephemeral), TOCTOU-shrunk reservation, killed with the session (`Drop` в†’ `start_kill`, `kill_on_drop`) | only while a debug session runs |
| Integrated terminal | in-process PTY (`aspect-terminal`) | none |
...
## The ladder вЂ” in order of preference
...
3. **`127.0.0.1` + ephemeral port + token, as a last resort.** If a component
   ...
   - bind `127.0.0.1:0` вЂ” never `0.0.0.0`, never a fixed port;
   - keep the listener's lifetime as short as the feature allows and tear it
     down with the owning session;
   - if AspectIDE itself serves the socket, require a per-launch random token on
     every request (write `port + token` to a `0600` file under app data вЂ”
...
- `aspect-dap/src/workspace.rs::sort_adapters_by_selection_priority` вЂ” when
  several adapters could serve a configuration, an installed **stdio** adapter
  is chosen over a TCP one (regression-tested).
- `aspect-dap/src/session.rs` вЂ” TCP adapters get an ephemeral loopback port with a
...
- `ai_turn.rs` (`browser_is_side_effecting`) and
  `aiRuntimeBrowser.ts::browserDashboardTool` вЂ” `BrowserDashboard start/stop`
