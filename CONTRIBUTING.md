# Contributing to AspectIDE

AspectIDE is a Rust-first desktop IDE. Contributions should make the editor more real, faster, clearer, or easier to maintain. Avoid decorative rewrites, mock-only features, and placeholder product flows.
...
- IPC must stay typed through Tauri commands, generated bindings, and `aspect://event` payloads.
...
Frontend DTOs are generated from Rust types in `crates/aspect-core`. When a cross-boundary type changes, regenerate/check bindings through the crate tests before frontend typechecking. Do not manually mirror DTOs in React.
...
Unless explicitly marked otherwise, contributions are submitted under the MIT License, matching this repository.
