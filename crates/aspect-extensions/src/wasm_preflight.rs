...
use crate::{
    ...
    EXTENSION_HOST_MAX_OUTPUT_BYTES, EXTENSION_HOST_MAX_TABLE_ELEMENTS, ASPECT_EXTENSION_ABI_VERSION,
    ASPECT_EXTENSION_ENTRYPOINT, ASPECT_EXTENSION_OPTIONAL_EXPORTS, ASPECT_HOST_IMPORT_MODULE,
...
pub fn validate_wasm_host_contract(
    ...
    if !exported_entrypoint {
        return Err(AppError::Service(format!(
            "WASM module must export required Aspect extension entrypoint: {ASPECT_EXTENSION_ENTRYPOINT}"
        )));
    }
    ...
    Ok(ExtensionHostActivationContract {
        abi: ExtensionWasmAbi {
            version: ASPECT_EXTENSION_ABI_VERSION,
            entrypoint: ASPECT_EXTENSION_ENTRYPOINT.to_string(),
            required_exports: vec![ASPECT_EXTENSION_ENTRYPOINT.to_string()],
            optional_exports: ASPECT_EXTENSION_OPTIONAL_EXPORTS
                .iter()
                .map(|v| (*v).to_string())
                .collect(),
            imports,
...
fn process_export_section(
    ...
    for export in reader {
        ...
        if export.name == ASPECT_EXTENSION_ENTRYPOINT {
            if export.kind != ExternalKind::Func {
                return Err(AppError::Service(format!(
                    "required export {ASPECT_EXTENSION_ENTRYPOINT} must be a function"
                )));
            }
...
/// functions are not yet implemented вЂ” they would always trap. Rather than
...
fn validate_host_import(
    ...
    if module != ASPECT_HOST_IMPORT_MODULE {
        return Err(AppError::Service(format!(
    ...
    let (TypeRef::Func(type_index) | TypeRef::FuncExact(type_index)) = ty else {
        return Err(AppError::Service(format!(
            "AspectIDE host import must be a function: {module}.{name}"
        )));
    };

    // F3 fix: validate the exact ABI signature.  Every linked AspectIDE host function
    ...
    let arity = func_type_arity
        ...
        .ok_or_else(|| {
            AppError::Service(format!(
                "AspectIDE host import {module}.{name} references an undefined or non-function type \
                 (index {type_index})"
            ))
        })?;
    if arity.params != 0 || arity.results != 0 {
        return Err(AppError::Service(format!(
            "AspectIDE host import {module}.{name} has unsupported signature \
    ...
    let Some(spec) = ALLOWED_HOST_IMPORTS
        ...
        return Err(AppError::Service(format!(
            "unsupported AspectIDE host import: {module}.{name}"
        )));
    };
    ...
    if spec.permission.is_some() {
        return Err(AppError::Service(format!(
            "AspectIDE host import '{name}' is declared in the contract but is not yet implemented; \
