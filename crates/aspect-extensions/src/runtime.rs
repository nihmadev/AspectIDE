...
use crate::{
    ...
    EXTENSION_WASM_EXECUTION_EXHAUSTED_FUEL, ASPECT_HOST_IMPORT_MODULE, WASM_PAGE_BYTES,
};
...
fn define_host_imports(
    ...
    for import in &contract.abi.imports {
        if import.module != ASPECT_HOST_IMPORT_MODULE
            || import.kind != aspect_core::ExtensionWasmImportKind::Function
        {
        ...
        // F9: I/O import arms are safety nets вЂ” preflight should have blocked
        // any extension that uses them before we ever get here.
        match import.name.as_str() {
            "log" => linker.func_wrap(ASPECT_HOST_IMPORT_MODULE, "log", || ()),
            "workspace_read" => linker.func_wrap(ASPECT_HOST_IMPORT_MODULE, "workspace_read", || {
                Err::<(), _>(wasmtime::format_err!("workspace_read is not implemented"))
            }),
            "workspace_write" => {
                linker.func_wrap(ASPECT_HOST_IMPORT_MODULE, "workspace_write", || {
                    Err::<(), _>(wasmtime::format_err!("workspace_write is not implemented"))
                })
            }
            "network_fetch" => linker.func_wrap(ASPECT_HOST_IMPORT_MODULE, "network_fetch", || {
                Err::<(), _>(wasmtime::format_err!("network_fetch is not implemented"))
            }),
            "process_spawn" => linker.func_wrap(ASPECT_HOST_IMPORT_MODULE, "process_spawn", || {
                Err::<(), _>(wasmtime::format_err!("process_spawn is not implemented"))
            }),
            name => {
                return Err(AppError::Service(format!(
                    "runtime refused unknown Aspect host import: {name}"
                )));
            }
        }
...
pub fn execution_failure_reason(
    error: &AppError,
    phase: aspect_core::ExtensionCommandExecutionPhase,
) -> String {
    use aspect_core::ExtensionCommandExecutionPhase::{Activation, Handler};
    match (phase, error) {
