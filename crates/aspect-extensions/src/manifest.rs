...
use crate::{ASPECT_EXTENSION_ENTRYPOINT, ASPECT_EXTENSION_OPTIONAL_EXPORTS, MAX_CONTRIBUTION_POINTS};
...
/// `aspect.foo` to register `aspect.foobar.run` (prefix impersonation of the
/// `aspect.foobar` namespace).  We now require `"{extension_id}."` (dot
...
fn validate_handler_export_name(handler: &str) -> AppResult<()> {
    ...
    if handler == ASPECT_EXTENSION_ENTRYPOINT || ASPECT_EXTENSION_OPTIONAL_EXPORTS.contains(&handler) {
        return Err(AppError::Service(format!(
            "extension command handler uses reserved AspectIDE export: {handler}"
        )));
    }
    if !handler
        .bytes()
