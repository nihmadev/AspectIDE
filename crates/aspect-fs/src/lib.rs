...
use aspect_core::{acquire_scan_workers, AppError, AppResult, FsEntry, FsEntryKind, ScanWorkers};
...
/// `.git/` or `.venv/`). Components of `root` itself are never considered вЂ” only the
/// portion the walk descended into вЂ” so opening a workspace whose own folder starts
...
/// Parallel full-tree crawl (no ignore/hidden filtering вЂ” mirrors a raw recursive
...
pub fn read_tree_bounded(root: impl AsRef<Path>, max_entries: usize) -> TreeScan {
    ...
    // clone survives вЂ” reclaim the buffer without cloning.
...
/// [`AtomicUsize`] counts every matching file the parallel walk encounters вЂ” including
/// the ones the bounded heap evicts вЂ” so the flag is exact regardless of which paths
...
pub fn list_files_scanned(root: impl AsRef<Path>, max_results: usize) -> FileListing {
    ...
    // `run` has returned, so every worker thread is joined and no other `Arc`
    // clone survives вЂ” reclaim the heap without cloning, then emit in path order.
...
/// accept arbitrary paths and run destructive `std::fs` operations directly вЂ” fine
...
impl WorkspaceFs {
    ...
    /// must succeed and produce the `notes/` folder вЂ” the bare `create_file` free
...
/// target (a file we are about to create) still validates вЂ” only `root` must exist
/// to be canonicalized.
...
fn copy_dir_recursive(from: &Path, to: &Path) -> AppResult<()> {
    ...
    for entry in fs::read_dir(from)? {
        ...
        // the destination вЂ” neither is caught by the one-shot canonicalize guard
