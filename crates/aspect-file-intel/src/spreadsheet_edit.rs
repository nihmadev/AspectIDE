...
pub const SPREADSHEET_EDIT_FORMAT: &str = "aspect-spreadsheet/v1";
...
pub fn spreadsheet_write_from_text(path: &Path, text: &str) -> AppResult<PathBuf> {
    ...
    if document.truncated {
        return Err(AppError::Service(
            "refusing to overwrite: workbook was truncated on load вЂ” \
        ...
        if size > MAX_LEGACY_SPREADSHEET_BYTES {
            return Err(AppError::Service(format!(
                "spreadsheet rejected: file size {} exceeds the {} MiB ceiling \
                 for non-zip workbooks вЂ” open externally for large legacy files",
