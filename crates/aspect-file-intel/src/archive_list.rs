...
use aspect_core::{AppError, AppResult, ArchiveEntryPreview};
...
pub fn list_archive_entries(path: &Path, max_entries: usize) -> AppResult<ArchiveListing> {
    let extension = aspect_core::file_extension_for_path(path);
...
fn list_compressed_tar_entries(
    ...
    // Standalone .gz/.bz2/.xz вЂ” expose as a single logical entry.
