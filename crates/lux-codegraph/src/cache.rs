...
//! every source file. On a giant workspace — a monorepo of hundreds of nested
//! projects — that is a multi-second, all-cores-pegged operation, and without a
...
//! reuses every entry whose fingerprint still matches — reparsing only the files
...
//! to parsing — and rebuilding keeps node ids, resolution, and the CSR adjacency
...
//!
//! ## Validation
//! A file is reused when its on-disk `(size, mtime)` equals the cached value —
...
//!
//! Caveat: on coarse-granularity filesystems (FAT/exFAT ≈ 2 s, some SMB/NFS mounts)
...
//!
//! ## Safety
//! * **Version-gated** — [`CACHE_VERSION`] is bumped whenever the on-disk layout,
//!   the parse output, or a grammar changes, so a stale-format cache is ignored
//!   rather than misread into a wrong graph.
//! * **Root-gated** — the cache records the root it was built for; a mismatch is
//!   ignored (defends against a copied or relocated `.lux`).
//! * **Corruption-tolerant** — any I/O or decode error yields `None` (→ a full
//!   rebuild), never a panic.
//! * **Atomic writes** — the cache is written to a sibling temp file and renamed
...
/// `MAX_FILE_BYTES`/admission policy, or a tree-sitter grammar *binary* — otherwise
...
/// they change — currently the tree-sitter `tags` query sources, which are edited
...
/// releases and platforms — a cache fingerprint must be reproducible.)
...
impl FileMeta {
    ...
    /// be stat'd or its mtime is unavailable — such a path is neither reused nor
    /// admitted (it may have just been deleted, or sit on a filesystem without
    /// modification times).
    #[must_use]
    pub fn of(path: &Path) -> Option<Self> {
        let metadata = std::fs::metadata(path).ok()?;
        // A pre-epoch mtime (`duration_since` errors) is "unknown" — treat it like a
...
/// skew, query-fingerprint change, root mismatch, or decode error yields `None` —
...
/// than failing the whole save — one oddly-named file never disables caching for
...
pub fn save<'a>(
    ...
    // another's in-progress write. The rename stays on the same volume — atomic,
    // replace-existing on Windows — so each writer installs its own complete file
...
/// so sibling `.lux` content (project skills/extensions) is untouched. Never
...
mod tests {
    use super::{
        load, query_fingerprint, save, DiskCacheRef, DiskEntryRef, FileMeta, PriorCache,
        CACHE_VERSION,
    };
    use crate::parse::{ParsedFile, RawSymbol, Span, SymbolKind};
    ...
    /// A unique scratch directory per call so parallel tests never collide.
    fn scratch() -> PathBuf {
    ...
    fn round_trips_entries() {
        ...
        let cache = dir.join("code-graph.bin");
        let path = root.join("a.rs");
        let meta = FileMeta {
            size: 12,
            mtime_ns: 345,
        };
        let parsed = sample("alpha");

        save(
            &cache,
            &root,
            std::iter::once((path.as_path(), meta, &parsed)),
        )
        .unwrap();

        let loaded = load(&cache, &root).expect("cache present");
        assert_eq!(loaded.len(), 1);
        let (got_meta, got_parsed) = loaded.get(&path).expect("entry present");
        assert_eq!(*got_meta, meta);
        assert_eq!(got_parsed.symbols[0].name, "alpha");

        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn rejects_a_different_root() {
        let dir = scratch();
        let root = dir.join("ws");
        let cache = dir.join("code-graph.bin");
        let parsed = sample("alpha");
        ...
        // A cache built for `root` must not be served for a different root.
        assert!(load(&cache, &dir.join("other")).is_none());
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn tolerates_corruption_and_absence() {
        let dir = scratch();
        let root = dir.join("ws");
        // Absent file → None.
        assert!(load(&dir.join("missing.bin"), &root).is_none());
        // Garbage bytes → None, never a panic.
        let garbage = dir.join("garbage.bin");
        std::fs::write(&garbage, b"not a postcard stream at all").unwrap();
        assert!(load(&garbage, &root).is_none());
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn prior_cache_is_a_plain_map() {
        // The reuse pass treats the loaded cache as a map; sanity-check the alias.
        let mut prior = PriorCache::default();
        prior.insert(
            PathBuf::from("x.rs"),
            (
                FileMeta {
    ...
    /// Forge a cache file with chosen `version`/`query_fp` so the load-gate can be
    /// exercised without round-tripping through `save` (which always stamps the
    /// current values).
    fn write_forged(cache: &std::path::Path, root: &std::path::Path, version: u32, query_fp: u64) {
        let path = root.join("a.rs");
        let parsed = sample("alpha");
        let disk = DiskCacheRef {
            version,
        ...
        std::fs::write(cache, postcard::to_allocvec(&disk).unwrap()).unwrap();
    }
    ...
    fn rejects_a_stale_version() {
        ...
        let root = dir.join("ws");
        let cache = dir.join("code-graph.bin");
        // A future format version we must refuse to read (postcard is not
        // self-describing, so a layout drift would otherwise be misread).
        write_forged(&cache, &root, CACHE_VERSION + 1, query_fingerprint());
        assert!(
            load(&cache, &root).is_none(),
            "stale version must be rejected"
        );
        // The current version with the right fingerprint loads.
        write_forged(&cache, &root, CACHE_VERSION, query_fingerprint());
        assert!(load(&cache, &root).is_some());
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn rejects_a_changed_query_fingerprint() {
        let dir = scratch();
        let root = dir.join("ws");
        let cache = dir.join("code-graph.bin");
        // Correct version but a tags-query fingerprint that no longer matches (as if
        // a `.scm` was edited) → discard the whole cache, no manual version bump.
        write_forged(
            &cache,
            &root,
            CACHE_VERSION,
            query_fingerprint() ^ 0xdead_beef,
        );
        assert!(
            load(&cache, &root).is_none(),
            "changed query fingerprint must be rejected"
        );
        std::fs::remove_dir_all(&dir).ok();
    }

    #[test]
    fn query_fingerprint_is_stable_and_nonzero() {
        // Reproducible within a build (a cache fingerprint must be), and not the
        // trivial seed.
        assert_eq!(query_fingerprint(), query_fingerprint());
        assert_ne!(query_fingerprint(), 0);
    }
}
