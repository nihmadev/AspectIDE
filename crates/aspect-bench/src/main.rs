...
fn main() -> BenchResult<()> {
    ...
    if options.assert_thresholds && !report.passed {
        return Err("AspectIDE performance benchmark thresholds failed".into());
    }
        ...
        match arg.as_str() {
            ...
            other => {
                if let Some(path) = other.strip_prefix("--output=") {
                    ...
                    return Err(format!("unknown aspect-bench argument: {other}").into());
                }
            }
        }
    }
...
fn benchmark_list_files(root: &Path) -> BenchResult<BenchmarkMetric> {
    ...
    let files = aspect_fs::list_files(root, EXPECTED_LISTED_FILES + 100)?;
...
fn benchmark_search(root: &Path) -> BenchResult<BenchmarkMetric> {
    ...
    let response = aspect_search::query(root, "aspect_target_symbol".to_string(), &options)?;
...
impl BenchmarkFixture {
    fn create() -> BenchResult<Self> {
        let root = env::temp_dir().join(format!("aspect-bench-{}", Uuid::new_v4()));
...
fn write_fixture(root: &Path) -> BenchResult<()> {
    ...
    fs::write(
        ...
        "pub const IGNORED: &str = \"aspect_target_symbol\";\n",
    )?;
    Ok(())
}
