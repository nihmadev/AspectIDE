...
//!
//! Everything here is pure data вЂ” no I/O. The desktop glue owns the actual
...
///
/// AspectIDE deliberately never exposes the unsafe `StrictHostKeyChecking=no`/`off`
...
pub struct SshOptions {
    /// `ConnectTimeout` (seconds) вЂ” how long the TCP/auth handshake may take
...
///
/// AspectIDE drives one short-lived, non-interactive `ssh` invocation per command, so a
/// "session" is the stored destination + a sticky logical working directory вЂ”
