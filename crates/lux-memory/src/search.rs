...
mod tests {
    ...
    fn pinned_outranks_equal_unpinned() {
        assert!(blend_rrf(0.5, 0.5, 0.5, 0, true) > blend_rrf(0.5, 0.5, 0.5, 0, false));
    }

    #[test]
    fn frequent_recall_strengthens_rank() {
        // Ebbinghaus-style reinforcement: same memory, more recalls → higher rank.
        assert!(blend_rrf(0.5, 0.5, 0.5, 20, false) > blend_rrf(0.5, 0.5, 0.5, 0, false));
        // Saturates: 10_000 recalls is capped at the weight ceiling.
        assert!(frequency_weight(10_000) >= 1.0);
    ...
    #[test]
    fn dense_ranks_gives_ties_the_same_rank() {
        assert_eq!(dense_ranks(&[3.0, 3.0, 2.0, 1.0]), vec![1, 1, 2, 3]);
        assert_eq!(dense_ranks(&[]), Vec::<usize>::new());
    }

    #[test]
    fn rank_by_score_orders_best_first() {
        let ranks = rank_by_score(vec![
            ("a".to_string(), 0.2),
            ("b".to_string(), 0.9),
            ("c".to_string(), 0.9),
        ]);
    ...
    #[test]
    fn rrf_weights_renormalize_over_active_streams() {
        // Single active stream absorbs the full weight.
        let bm25_only = RrfWeights::renormalized(true, false, false);
        assert!((bm25_only.fuse(Some(1), None, None) - rrf_term(1)).abs() < 1e-9);
        // No active streams → zero contribution, never a panic/NaN.
        let none_active = RrfWeights::renormalized(false, false, false);
        assert_eq!(none_active.fuse(Some(1), Some(1), Some(1)), 0.0);
        // All three active: weights are proportional to the base weights.
        let all_active = RrfWeights::renormalized(true, true, true);
        let vector_only_score = all_active.fuse(None, Some(1), None);
        let bm25_only_score = all_active.fuse(Some(1), None, None);
        assert!(vector_only_score > bm25_only_score); // vector base weight (0.6) > bm25 (0.4)
    }

    #[test]
    fn jaccard_similarity_matches_expected_ratio() {
        let a = token_set("the project uses pnpm workspaces");
        let b = token_set("the project uses pnpm workspaces today");
        // {the,project,uses,pnpm,workspaces} ⊂ {..., today}: 5/6.
        assert!((jaccard_similarity(&a, &b) - (5.0 / 6.0)).abs() < 1e-9);
        assert_eq!(jaccard_similarity(&HashSet::new(), &HashSet::new()), 0.0);
    }

    #[test]
    fn retention_tiers_are_ordered() {
        assert_eq!(retention_tier(0.95), RetentionTier::Hot);
        assert_eq!(retention_tier(0.5), RetentionTier::Warm);
        assert_eq!(retention_tier(0.2), RetentionTier::Cold);
        assert_eq!(retention_tier(0.0), RetentionTier::Evictable);
    }

    #[test]
    fn retention_score_decays_with_age_and_idleness() {
        let base = Memory {
            id: "m".into(),
        ...
        let now = 0;
        let fresh_score = retention_score(&base, now);
        let stale = Memory {
            last_accessed_at: -400 * 86_400_000,
            created_at: -400 * 86_400_000,
            ..base.clone()
        };
        let stale_score = retention_score(&stale, now);
        ...
        assert!(retention_score(&recalled, now) > fresh_score);
    }

    #[test]
    fn infer_relation_confidence_applies_expected_nudges() {
        let recent = Memory {
            id: "a".into(),
            category: "core".into(),
            content: "x".into(),
        ...
        let now = 0;
        // Both updated "now" (within 7 days) → +0.1 over the 0.5 baseline.
        let related = infer_relation_confidence(&recent, &recent, RelationKind::Related, now);
        assert!((related - 0.6).abs() < 1e-9);
        // Supersedes stacks its own +0.1 on top.
        let supersedes = infer_relation_confidence(&recent, &recent, RelationKind::Supersedes, now);
        assert!((supersedes - 0.7).abs() < 1e-9);
        // Contradicts nudges down instead.
        let contradicts =
            infer_relation_confidence(&recent, &recent, RelationKind::Contradicts, now);
        assert!((contradicts - 0.55).abs() < 1e-9);
        // Both stale (>90 days idle, and not updated within 7 days) → -0.1, clamped to [0,1].
        let stale = Memory {
            last_accessed_at: -100 * 86_400_000,
            updated_at: -100 * 86_400_000,
            ..recent
        };
        let stale_confidence =
            infer_relation_confidence(&stale, &stale, RelationKind::Related, now);
        assert!((stale_confidence - 0.4).abs() < 1e-9);
    }
}
