...
//! `lux-ssh` — the pure core of Lux's first-class SSH integration.
//!
...
//!   argument vectors — the fix for agents that hang on host-key/password
...
/// directory — Lux runs one short-lived `ssh` process per command, so there is no
...
mod tests {
    ...
    fn registry_insert_get_list_remove() {
        ...
        let a = registry.insert("a".into(), target(), String::new(), None, None);
        let b = registry.insert("b".into(), target(), "/srv".into(), None, None);
        assert_eq!(registry.count(), 2);
        assert_eq!(registry.get(a.id).unwrap().label, "a");
        assert_eq!(registry.get(b.id).unwrap().cwd, "/srv");
    ...
    #[test]
    fn registry_set_cwd_and_clear() {
        let registry = SshRegistry::new();
        let s = registry.insert("s".into(), target(), String::new(), None, None);
        assert!(registry.set_cwd(s.id, "/var/www".into()));
        assert_eq!(registry.get(s.id).unwrap().cwd, "/var/www");
        assert!(!registry.set_cwd(Uuid::new_v4(), "/x".into()));
