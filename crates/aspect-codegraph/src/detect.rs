...
/// A most-connected "core abstraction" вЂ” the symbols everything leans on.
...
///
/// Nodes whose only edges are structural nesting (`Defines`) are skipped вЂ” they
...
/// True when every edge touching `node` is a structural `Defines` edge вЂ” i.e. the
...
/// A connection that isn't obvious from file layout вЂ” a cross-file edge between
...
/// rewards low confidence, cross-community bridges, and peripheralв†’hub links вЂ”
...
pub fn surprising_connections(
    ...
    for i in 0..graph.node_count() {
        ...
        for adj in graph.out_neighbors(from) {
            // Structural edges are obvious by construction вЂ” never surprising.
...
/// Mirrors `_surprise_score`: confidence weight + cross-community + peripheralв†’hub.
fn surprise_score(
    graph: &CodeGraph,
    u: NodeId,
    v: NodeId,
    ...
    // 1. Confidence weight вЂ” uncertain connections are more noteworthy.
    ...
    if confidence != Confidence::Extracted {
        reasons.push(format!(
            "{} connection вЂ” not a direct same-file reference",
            confidence.tag().to_lowercase()
        ));
    }

    // 2. Cross-community bridge вЂ” the partitioner placed these far apart.
    ...
    // 3. Peripheral в†’ hub: a low-degree node reaching a high-degree one.
...
/// component of size в‰Ґ 2 in that file graph is a cycle. One representative cycle
...
/// analysis from [`import_cycles`] вЂ” call cycles are not the same thing as import
/// cycles. Returns one [`ImportCycle`] (reusing the type for convenience) per
/// strongly-connected component of size в‰Ґ 2 in the file-level call/reference
/// adjacency.
#[must_use]
...
/// Build fileв†’file SCCs restricted to the given edge kinds.
