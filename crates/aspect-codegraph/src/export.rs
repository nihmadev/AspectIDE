...
//! Python's `json.dump(indent=2, ensure_ascii=True, sort_keys=False)` вЂ” 2-space
//! indent, non-ASCII escaped to `\uXXXX`, keys in insertion order, no trailing
//! newline вЂ” so the output drops into graphify's `graphify-out/` and is readable
...
/// same-named defs share a line (macro-generated / overloaded items) вЂ” a plain
...
pub fn to_graph_json(graph: &CodeGraph, communities: &[Community], root: &Path) -> String {
    ...
    for i in 0..graph.node_count() {
        ...
        // Node object вЂ” key order matches export.py: stored attrs, id, community,
    ...
    for i in 0..graph.node_count() {
        ...
        for adj in graph.out_neighbors(from) {
            // Edge object вЂ” relation/confidence/source_file then source/target,
...
/// nodes вЂ” an O(nВІ)-per-frame force layout in the browser bogs down past a few
...
/// renderer вЂ” no network, no CDN, no third-party script вЂ” so it opens offline and
...
pub fn to_graph_html(
    ...
    for (from, _) in &all {
        ...
        for adj in graph.out_neighbors(from) {
            if !kept.contains(&adj.node) {
                continue; // edge to a trimmed node вЂ” drop it
            }
    ...
    .to_string_pretty();
    // `</` в†’ `<\/` keeps the data from prematurely closing the <script> tag while
...
pub fn to_report(graph: &CodeGraph, communities: &[Community], title: &str, date: &str) -> String {
    ...
    let _ = writeln!(
        out,
        "- Extraction: {}% EXTRACTED В· {}% INFERRED В· {}% AMBIGUOUS",
    ...
    // в”Ђв”Ђ Surprising connections в”Ђв”Ђ
    let _ = writeln!(
        out,
